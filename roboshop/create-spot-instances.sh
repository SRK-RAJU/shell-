...
if [ $COUNT -eq 0 ]; then
aws ec2 run-instances --launch-template LaunchTemplateId=lt-05637ecd56f2d04c6,Version=1 --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=$1}]" "ResourceType=spot-instances-request,Tags=[{Key=Name,Value=$1}]" | jq &>/dev/null
else
